...
class Payroll extends MY_Controller {
	...
	public function add_pay_monthly() {
		...
		'sss_deduction'              => $this->input->post('sss_deduction'),
		'hdmf_deduction'             => $this->input->post('hdmf_deduction'),
		'philhealth_deduction'       => $this->input->post('philhealth_deduction'),
		'tax_income_deduction'       => $this->input->post('tax_income_deduction'),
		'dsca_total'                 => $this->input->post('dsca_total'),
		'is_payment'                 => '1',
	...
	// payment history
	 public function payslip()
     {
		...
		$data['sss_deduction']        = $result[0]->sss_deduction;
		$data['hdmf_deduction']       = $result[0]->hdmf_deduction;
		$data['philhealth_deduction'] = $result[0]->philhealth_deduction;
		$data['tax_deduction']        = $result[0]->tax_income_deduction;
	 ...
	 public function pdf_create(){
		...
		$sss_deduction        = $payment[0]->sss_deduction;
		$hdmf_deduction       = $payment[0]->hdmf_deduction;
		$philhealth_deduction = $payment[0]->philhealth_deduction;
		$tax_deduction        = $payment[0]->tax_income_deduction;
		...
		if($payment[0]->payslip_type=='hourly'){
			...
			$total_deductions = $loan_de_amount + $statutory_deduction_amount + ($sss_deduction + $philhealth_deduction + $hdmf_deduction + $tax_deduction);
		}
		/*<tr>
		...
		$tblbrk = '<table cellpadding="3" cellspacing="0" border="1"><tr bgcolor="#69e48a">
			...
			// ** Government Deductions *** //
				//** SSS Deduction ** //
				$tblbrk .= '<tr>
					<td colspan="2">SSS Deduction</td>
					<td>&nbsp;</td>				
					<td align="center"  valign="bottom">'.$this->Xin_model->currency_sign($sss_deduction).'</td>				
				</tr>';
				//** Philhealth Deduction ** //
				$tblbrk .= '<tr>
					<td colspan="2">Philhealth Deduction</td>
					<td>&nbsp;</td>				
					<td align="center"  valign="bottom">'.$this->Xin_model->currency_sign($philhealth_deduction).'</td>				
				</tr>';

				//** HDMF Deduction ** //
				$tblbrk .= '<tr>
					<td colspan="2">HDMF Deduction</td>
					<td>&nbsp;</td>				
					<td align="center"  valign="bottom">'.$this->Xin_model->currency_sign($hdmf_deduction).'</td>				
				</tr>';

				//** TAX Deduction ** //
				$tblbrk .= '<tr>
					<td colspan="2">Withholding Tax Deduction</td>
					<td>&nbsp;</td>				
					<td align="center"  valign="bottom">'.$this->Xin_model->currency_sign($tax_deduction).'</td>				
				</tr>';
			...
			$tblbrk .= '
				...
				$total_deduction = $loan_de_amount + $statutory_deduction_amount + ($sss_deduction + $philhealth_deduction + $hdmf_deduction + $tax_deduction);
				$total_net_salary = $total_earning - $total_deduction;
				$tblbrk .= '
					<tr><td colspan="2" align="center"><strong>Total</strong></td>
	...
	public function dsca_adjustment(){
	 $session = $this->session->userdata('username');
		if(empty($session)){ 
			redirect('admin/');
		}
		...
		$data['all_employees'] = $this->Xin_model->all_employees();
		$data['breadcrumbs'] = $this->lang->line('left_generate_payslip');
		$data['path_url'] = 'DSCA Adjustment';
		$role_resources_ids = $this->Xin_model->user_role_resource();
		if(in_array('36',$role_resources_ids)) {
			if(!empty($session)){ 
				$data['subview'] = $this->load->view("admin/payroll/dsca_adjustment", $data, TRUE);
				$this->load->view('admin/layout/layout_main', $data); //page load
			} else {
				redirect('admin/');
			}
